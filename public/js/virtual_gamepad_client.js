...
var initJoystick = function () {
  ...
  setInterval(
    function () {
      ...
      if (motionListener) {
        var normalizedX = Math.min(1, Math.max(-1, xPosition));
        var normalizedY = Math.min(1, Math.max(-1, yPosition));

        if (analog) {
          var xy = {
            x: Math.round(127 * (normalizedX + 1)),
            y: Math.round(127 * (normalizedY + 1)),
          };
          setDirection(xy);
        } else {
          var direction = "";
        ...
        if (analog) {
          ...
          if (
            ...
            if (gamepad != null && (gamepad.xAxis || gamepad.yAxis)) {
              if (gamepad.xAxis != null)
                xy.x = Math.round((255.9999 * (gamepad.xAxis + 1)) / 2 - 0.5);
              if (gamepad.yAxis != null)
                xy.y = Math.round((255.9999 * (gamepad.yAxis + 1)) / 2 - 0.5);
            }
            setDirection(xy);
...
$(function () {
  ...
  var states = [
    ...
    {
      type: "button",
      text: "<p>Press <code>UP</code> on the D-Pad",
      mapName: "dpadUP",
    },
    {
      type: "button",
      text: "<p>Press <code>RIGHT</code> on the D-Pad",
      mapName: "dpadRIGHT",
    },
    {
      type: "button",
      text: "<p>Press <code>DOWN</code> on the D-Pad",
      mapName: "dpadDOWN",
    },
    {
      type: "button",
      text: "<p>Press <code>LEFT</code> on the D-Pad",
      mapName: "dpadLEFT",
    },
    {
      type: "button",
      text: "<p>Press the <code>X</code> Button (top button)",
      mapName: "btnX",
    },
    {
      type: "button",
      text: "<p>Press the <code>A</code> Button (right button)",
      mapName: "btnA",
    },
    {
      type: "button",
      text: "<p>Press the <code>B</code> Button (bottom button)",
      mapName: "btnB",
    },
    {
      type: "button",
      text: "<p>Press the <code>Y</code> Button (left button)",
      mapName: "btnY",
    },
    {
      type: "button",
      text: "<p>Press the <code>SELECT</code> Button",
      mapName: "btnSELECT",
    },
    {
      type: "button",
      text: "<p>Press the <code>START</code> Button",
      mapName: "btnSTART",
    },
    {
      type: "button",
      text: "<p>Press the <code>L</code> Button (left shoulder button)",
      mapName: "btnLT",
    },
    {
      type: "button",
      text: "<p>Press the <code>R</code> Button (right shoulder button)",
      mapName: "btnRT",
    },
    {
      type: "button",
      text: "<p>Press the <code>L2</code> Button",
      mapName: "btnLT2",
    },
    {
      type: "button",
      text: "<p>Press the <code>R2</code> Button",
      mapName: "btnRT2",
    },
    {
      type: "axis",
      text: "<p>Move the analog stick <code>right</code>",
      mapName: "x",
    },
    {
      type: "axis",
      text: "<p>Move the analog stick <code>down</code>",
      mapName: "y",
    },
  ];
...
var buttonMap = (localStorageAvailable &&
...
var axesNormalizeStats =
  (localStorageAvailable &&
    JSON.parse(
      window.localStorage.getItem("gamepadAxesNormalizeStats") || "null",
    )) ||
  {};
function normalizeAxis(axisId, value) {
  ...
  if (changed && localStorageAvailable) {
    window.localStorage.setItem(
      "gamepadAxesNormalizeStats",
      JSON.stringify(axesNormalizeStats),
    );
  }
    ...
    for (i = 0; i < controller.axes.length; i++) {
      ...
      if (rebindModal.isOpen()) {
        ...
        if (typeof mapped === "string") {
          ...
          handleGamepadButton(controller, mapped[0], val < -0.5);
          handleGamepadButton(controller, mapped[1], val > 0.5);
        }
      }
    }
  }
...
var initSlotIndicator = function () {
  ...
  var slotAnimationLoop = function () {
    if (ledBitField != null) {
      ...
      if (ledBitField & b0001) {
        $("#indicator_1").addClass("indicatorSelected");
      }
      if (ledBitField & b0010) {
        $("#indicator_2").addClass("indicatorSelected");
      }
      if (ledBitField & b0100) {
        $("#indicator_3").addClass("indicatorSelected");
      }
      if (ledBitField & b1000) {
        $("#indicator_4").addClass("indicatorSelected");
      }
    } else {
      if (indicatorOn) {
        $(".indicator").removeClass("indicatorSelected");
...
const gyroSlider = document.getElementById("slider-gyro");
const gyroSliderLabel = document.getElementById("slider-gyro-label");
const gyroSliderDiv = document.getElementById("slider-gyro-div");
const barLeft = document.getElementById("accelerataion-bar-left");
const barRight = document.getElementById("accelerataion-bar-right");
...
function orientationUpdate(event) {
  ...
  if (beta > 0 && gamma < 20) {
    xPosition = beta / 70 * sensivity;
  } else if (beta > 0 && gamma > 20) {
    xPosition = (180 - beta) / 70 * sensivity;
  } else if (beta < 0 && gamma < 20) {
    xPosition = beta / 70 * sensivity;
  } else if (beta < 0 && gamma > 20) {
    xPosition = -(180 + beta) / 70 * sensivity;
  ...
  if (gamma < 0) {
    yPosition = ((gamma + 50) / 40) * sensivity;
  }

  if (xPosition > 0) {
    barLeft.style.width = xPosition * 100 + "%";
    barRight.style.width = 0;
  } else {
    barRight.style.width = -xPosition * 100 + "%";
    barLeft.style.width = 0;
  }

  // testDiv.innerHTML = `alpha: ${alpha}<br>beta: ${beta}<br>betaPos: ${xPosition}<br>gamma: ${gamma}`;
}
...
function motionUpdate(event) {
  ...
  xPosition = (Math.atan2(y, Math.abs(z)) / 1.4) * sensivity;
  yPosition = ((Math.atan2(x, Math.abs(z)) / .6) - 1.6) * sensivity;
  ...
  // testDiv.innerHTML = `X: ${x} <br>Y: ${y}<br>Z: ${z.toFixed(2)}`;
}

gyroSlider.addEventListener("input", () => {
  sensivity = gyroSlider.value;
  gyroSliderLabel.innerHTML = Number(gyroSlider.value).toFixed(1);
});
